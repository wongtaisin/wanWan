...
class ExpensesService {
  // 查询所有
  expensesAll = `SELECT * FROM expenses`
   ...
   */
  filterUserIdAndDate = (row: { userId?: number; startDate?: string; endDate?: string }) => {
  ...
  // 添加
  addExpenses = `INSERT INTO expenses (user_id, user_name, eat, drink, play, glad, tolls, oil, parking, traffic, supermarket, online_shopping, phone_bill, red_packet, vip, other, create_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, COALESCE(NULLIF(?, ''), now()))`
   ...
   */
  updateExpenses = `UPDATE expenses SET eat = ?, drink = ?, play = ?, glad = ?, tolls = ?, oil = ?, parking = ?, traffic = ?, supermarket = ?, online_shopping = ?, phone_bill = ?, red_packet = ?, vip = ?, other = ? WHERE id = ?`
   ...
   * @demo ['2,15', 1]
   */
  updateExpensesFieldName = (fieldName: string) => {
   ...
   */
  updateExpensesDate = (fieldName: string) => {
   ...
   */
  checkDate = `SELECT * FROM expenses WHERE DATE(create_date) = ?` // DATE(create_date) 是将 create_date 转换为日期格式，然后再进行比较
   ...
   */
  checkDateByUserId = `SELECT * FROM expenses WHERE DATE(create_date) = ? AND user_id = ?` // DATE(create_date) 是将 create_date 转换为日期格式，然后再进行比较，同时对比 user_id 是否相同
  ...
  // 删除id
  deleteExpensesByUserIdAndDate = `DELETE FROM expenses WHERE user_id = ? AND DATE(create_date) = ?`
}
