 ...
 * @LastEditTime: 2025-12-31 15:22:01
 * @FilePath: \wanWan\src\service\loginService.ts
 * @Description:
 *
...
class loginService {
  ...
  checkUserAndPhone = `SELECT * FROM user WHERE user_name = ? OR phone = ?`

  // verifyUser = `SELECT * FROM user WHERE user_name = ? AND password = ?`

  addUser = `INSERT INTO user (user_name, phone, age, sex, password, salt,  create_time) VALUES (?, ?, ?, ?, ?, ?, now())`
