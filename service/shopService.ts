...
class ShopService {
  // 查询所有
  getIdShop = `SELECT * FROM shop WHERE 1=1 AND id = ?`
   ...
   *
   */
  addShop = `INSERT INTO shop (user_id, shop_name, province_code, province, city_code, city, area_code, area, address, images, remark, create_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now())`
   ...
   */
  editShop = `UPDATE shop SET shop_name = ?, province_code = COALESCE(?, province_code), province = COALESCE(?, province), city_code = COALESCE(?, city_code), city = COALESCE(?, city), area_code = COALESCE(?, area_code), area = COALESCE(?, area), address = COALESCE(?, address), images = COALESCE(?, images), remark = COALESCE(?, remark), update_date = NOW() WHERE id = ?`
   ...
   *
   */
  checkShopName = `SELECT * FROM shop WHERE shop_name = ?`
   ...
   * @demo [1]
   */
  deleteShopId = `DELETE FROM shop WHERE id = ?`
   ...
   *
   */
  checkShopUserId = `SELECT * FROM shop WHERE user_id = ? AND id = ?`
   ...
   *
   */
  getUserIdShop = `SELECT *, DATE_FORMAT(create_date, '%Y-%m-%d %H:%i:%s') AS create_date FROM shop WHERE 1=1 AND user_id = ? ORDER BY create_date DESC LIMIT ? OFFSET ?`
}
